                            ...
                            <tr class="search-row">
                                ...
                                <td class="text-center">
                                    <div class="d-inline-flex align-items-center justify-content-center gap-1">
                                        ...
                                        <!-- Owner/Manager Multi-Stage Payment Button (DP / Termin 2 / Pelunasan) -->
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                @forelse($purchases as $purchase)
                    <div class="o_kanban_record bg-white border rounded p-3 shadow-sm hover:shadow transition search-card" style="border-left: 4px solid {{ $purchase->status === 'received' ? '#059669' : ($purchase->status === 'waiting_approval' ? '#d97706' : '#2563eb') }} !important;">
                        ...
                        <div class="text-[11px] text-slate-500 mb-1">Vendor: {{ $purchase->supplier->name ?? 'N/A' }}</div>
                        
                        <!-- Payment Tag -->
                        <div class="mb-2">
                            @if($purchase->isPaid())
                                <span class="badge bg-emerald-100 text-emerald-800 text-[10px] font-bold">✓ LUNAS</span>
                            @elseif($purchase->isPartiallyPaid())
                                <span class="badge bg-amber-100 text-amber-800 text-[10px] font-bold">DP: Rp {{ number_format($purchase->paid_amount, 0, ',', '.') }}</span>
                            @else
                                <span class="badge bg-rose-50 text-rose-700 text-[10px]">Belum Dibayar</span>
                            @endif
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center pt-2 border-top border-slate-100 mt-2">
                            ...
                            <div class="d-flex gap-1">
                                @if((auth()->user()->isOwner() || auth()->user()->isSuperAdmin() || auth()->user()->isManager()) && !$purchase->isPaid())
                                    <button type="button" @click="openPayModal({{ json_encode($purchase) }})" class="btn btn-sm btn-outline-primary py-0 px-1.5 text-xs font-semibold">
