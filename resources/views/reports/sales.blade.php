        ...
        <form method="GET" action="{{ route('reports.sales') }}" class="row g-2 align-items-end">
            ...
            <div class="col-12 col-md-3">
                ...
                <select name="branch_id" class="form-select form-select-sm">
                    <option value="all" {{ ($selectedBranchId ?? 'all') === 'all' ? 'selected' : '' }}>Semua Cabang (Konsolidasi)</option>
                    @foreach($branches as $branch)
                        <option value="{{ $branch->id }}" {{ ($selectedBranchId ?? '') == $branch->id ? 'selected' : '' }}>{{ $branch->nama_cabang }}</option>
                    @endforeach
                </select>
            </div>
            @endif
